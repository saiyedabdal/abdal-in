 ...
 *
 * The public site needs no password. This function runs ONLY on /private and
 * the /api data endpoints and asks for the private password there — checked
 * at the edge so the workspace is never shipped to anyone without it (the
 * repo is public, so a browser-side check would leak the password).
 *
 * PRIVATE_PASSWORD is set with `netlify env:set` and never committed.
 */
...
export default async (request) => {
  ...
  // A lock that opens when it is confused is not a lock: fail closed.
  if (!secret) return new Response(misconfigured(), { status: 503, headers: htmlHeaders() });
  ...
  if (cookieIs(PRIVATE_COOKIE, token)) return; // already unlocked → serve normally

  // Only a form submission can be a login answer. The workspace API posts
  // JSON, and reading that body here would consume it before the function.
  ...
  if (form && String(form.get("stage") || "") === "private") {
    if (safeEqual(String(form.get("password") || ""), secret)) {
      const dest = String(form.get("next") || "/private");
      const to = dest.startsWith("/") && !dest.startsWith("//") ? dest : "/private";
      return new Response(null, {
        status: 303,
        headers: {
          location: to,
          "set-cookie":
            `${PRIVATE_COOKIE}=${token}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${MAX_AGE}`,
        },
      });
    }
    return new Response(privateGate(url.pathname, true), { status: 401, headers: htmlHeaders() });
  }

  // An API call without the cookie gets JSON, not a login page — the
  // workspace's own fetch() would otherwise choke on HTML.
...
<style>
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  button{
    ...
    background:#F7D842;color:#0B0B0B;border:0;transition:background .2s,transform .2s;
  }
...
function privateGate(next, wrong) {
  return shell(
    ...
    ${wrong ? '<p class="err">Not that one. This door takes the private password.</p>' : ""}
...
function misconfigured(which = "PRIVATE_PASSWORD") {
  return shell(
    "Private",
    `<span class="mark">Abdal</span>
    <div class="rule"></div>
    <h1>Locked</h1>
    <p class="lede">This door has no password configured, so it is holding shut
       rather than letting everyone in.</p>
    <p class="err">Set <strong>${esc(which)}</strong> in the Netlify project
       environment variables, then redeploy.</p>`
  );
}

// Runs only on the private workspace and its data API. The whole rest of the
// site is public and never touches this function.
export const config = { path: ["/private", "/private/*", "/api/*"] };
